...
bool XMJGHouse::addFunctionAnnotion(const AcDbObjectId &id, CString &info)
{
	...
	for (MSIdsIter it = fcpmgnqmsids.begin(); it != fcpmgnqmsids.end(); ++it)//遍历按照层高和计容系数分类的功能区
	{
		...
		if (sarea == 0.0) continue;//如果sarea为零，则不输出当前类别
		CString temp;
...
void XMJGHouse::removeSubFunction(const AcDbObjectId &id, double &jzArea, double &jrArea)
{
	...
	AcDbObjectIdArray dids;//所有岛ID集合
	TCHAR layer[255] = { 0 };//用于储存图层名称
	_stprintf(layer, _T("%s"), m_gnq._layer);//复制功能区图层
	ssFromNodes(dids, nodes, 1, 0.5, _T("*POLYLINE"), layer);//WP选择功能区实体
	dids.remove(id);
	for (int idx = 0; idx < dids.length(); idx++)//遍历岛功能区
	{
	...
	ssFromNodes(dids, nodes, 1, 0.5, _T("*POLYLINE"), layer);//WP选择车位实体
	for(int idx = 0; idx < dids.length(); ++idx)//遍历岛车位
	{
		AcDbObjectId did = dids.at(idx);
