...
XMJGHouse::XMJGHouse()
{
	...
	m_fcmjzs = oper.readTextPropery(_T("建筑竣工分层面积注释"), true, true);
	...
	oper.readCommonTable(_T("建筑竣工分层平面面积注记字体"), m_mjzj._font);
...
{
	CString name = GetCurrentDwgName();//获得当前的DWG文件名
	if(name.Find(_T("项目图形.dwg")) == -1)//判断当前DWG是否为项目图形文件
	{
	...
	pdb.setMJXXTableInfo(record);//插入或更新MJXX表中的关键字“实测建筑用地面积”对应的属性值
	XMJGAssist assist;
	CString savePath = assist.getExportFilePath();//获得xmjg.mdb中的XMJGPath中的当前目录属性值
	SmartTable::Word word; TCHAR path[255] = {0};
	if(RTNORM != ads_findfile(_T("竣工验收报告书.docx"), path))//在搜索路径中寻找该文件的文件全名
	{
...
void XMJGHouse::calculateJZZDMJ()
{
	...
	for (int i = 0; i < JZWLKArr.length(); i++)
	{
		...
		{
			AcDbHatch *pHatch;
			if (Acad::eOk != acdbOpenObject(pHatch, hatchArr.at(j), AcDb::kForRead))return;//以只读方式打开填充区域
...
{
	SelectFilter flayer(8, m_fcpm._layer);//创建用于筛选分层平面外边界线的图层筛选条件
	SelectFilter fename(RTDXF0, _T("*POLYLINE"));//创建用于筛选分层平面外边界线的线型筛选条件
	AcDbObjectIdArray fcpmids;
	if (false == SelectEntitys(fcpmids, flayer, fename, _T(""))) return;//手动选择并筛选所有分层平面，将它们的id号赋值给objects
	ads_point pInsert;//面积注释插入位置
	ads_point pRight;//面积注释显示右侧边界
	if (RTNORM != ads_getpoint(NULL, _T("\n请拾取文字横向显示左侧边界点："), pInsert)) return;//拾取面积注释插入位置
	if (RTNORM != ads_getpoint(pInsert, _T("\n请拾取文字横向显示右侧边界点："), pRight))return;//拾取面积注释显示右侧边界位置
	for(int idx = 0; idx < fcpmids.length(); ++idx)//遍历每个分层平面图
	{
		AcDbObjectId fcpmid = fcpmids.at(idx);//获得一个分层平面图的id号
		ads_point pmin, pmax;//分层平面左下角点和右上角点
		GetEntExtent(aname(fcpmid), pmin, pmax);//范围线左下角坐标、范围线右上角坐标
		ZoomWindow(pmin, pmax);//最大化分层平面图
		CString outputinfo;//面积注释文字内容
		outputinfo += _T("注:\t");
		//输出功能区信息
...
bool XMJGHouse::addFunctionAnnotion(const AcDbObjectId &id, CString &info)
{
	...
	CString LCH(lch);//将TCAHR字符串转化为CString，便于对字符串进行操作
	int cpos = LCH.Find(_T(" "));//找到楼层号中的空格的位置，空格左边是起始楼层号，右边是结束楼层号
	if (cpos != -1) _stprintf(lch, _T("%s"), LCH.Mid(0, cpos));//截取楼层号中的起始楼层号赋值给lch
	MStr filter;
	filter[_T("楼栋号")] = ldh;//在filter中创建“楼栋号”属性，并赋值为ldh
	filter[_T("起始层名")] = lch;//在filter中创建“起始层名”属性，并赋值为lch
	IProjectMDB pdb;
	MStr record = pdb.getCXXTableInfo(filter);//根据楼栋号和起始层名找到CXX表中的分层平面信息行，并赋值给info
	CString wbqcgjrxs;//当前分层平面的层高和计容系数
	if (lch[0] == _T('-'))//如果是地下楼层则计容系数为1
		wbqcgjrxs.Format(_T("层高%.2lfm，计容系数%.2lf"), _tstof(record[_T("实测层高")]), 0.00);
	else
		wbqcgjrxs.Format(_T("层高%.2lfm，计容系数%.2lf"), _tstof(record[_T("实测层高")]), 1.00);
	AcDbObjectIdArray fcpmgnqids;//当前分层平面中所有功能区的ID集合
	AcGePoint2dArray fcpmnodes;//分层平面边界线所有节点
	GetPlList(aname(id), fcpmnodes);//获得分层平面图的所有节点
	TCHAR layer[255] = { 0 };//存储图层名称
	_stprintf(layer, _T("%s"), m_gnq._layer);
	ssFromNodes(fcpmgnqids, fcpmnodes, 1, 1.0, _T("*POLYLINE"), layer);//获得分层平面图中的所有功能区的id集合
	MSIds fcpmgnqmsids = sortGNQByXData(fcpmgnqids);//排序所有功能区
	double dzdarea = 0.0;//用于累加所有非扣岛功能区面积和外半墙面积
	double wbqjzmj = 0.0, wbqjrmj = 0.0;//建筑面积wbqjzmj、计容面积wbqjrmj
	getWaiBanQiangArea(id, wbqjzmj, wbqjrmj);//将外半墙面积追加到jzmj和jrmj中
	bool wbqisAnn = fabs(wbqjzmj) == 0 ? true : false;//面积为0的外半墙不输出
	for (MSIdsIter it = fcpmgnqmsids.begin(); it != fcpmgnqmsids.end(); ++it)//遍历按照层高和计容系数分类的功能区
	{
		...
		{
			AcDbObjectId cjgnqid = cjgnqids.at(idx);//将当前功能区指定为序号idx指向功能区
			TCHAR cjisKouDao[255] = { 0 };
			ReadXdata(aname(cjgnqid), _T("扣岛"), 0, cjisKouDao);
			if (_tcscmp(cjisKouDao, _T("true")) == 0)continue;//扣岛为真的功能区不输出
			FunctionHAH cjhah;//用于存放当前功能区的编号、面积等信息
			getFunctionArea(cjgnqid, cjhah);//获得当前功能区的面积
			if (fabs(cjhah.m_jzmj) < EPS) continue;//如果当前功能区面积为零，则略去
			gnqmp.insert(std::make_pair(cjhah.m_bh, cjhah.m_jzmj));//当前功能区简码编号和面积组成值对插入进gnqmp中
		}
		//遍历储存功能区面积的map<CString, double, CmpCstr>
		for (std::map<CString, double, CmpCstr>::iterator itr = gnqmp.begin(); itr != gnqmp.end(); ++itr)
		{
			sarea += itr->second;//累加非扣岛功能区面积
			CString mpjmbh = itr->first;//当前功能区编号
			mpjmbh += _T("、");//罗列所有功能区简码编号，以中文顿号分隔
			info += mpjmbh;//将功能区简码编号罗列值追加到info中
		}
		if (!wbqisAnn)
		{
			if (it->first.CompareNoCase(wbqcgjrxs) == 0)
			{
				sarea += wbqjzmj;//累加外半墙面积
				info += _T("WBQ、");//将罗列值追加到info中
				wbqisAnn = true;//表示外半墙面积已经注释
			}
		}
		info = info.Mid(0, info.GetLength() - _tcslen(_T("、")));//去除尾部中文顿号
		if (sarea == 0.0) continue;//如果sarea为零，则不输出当前层高和计容系数下的功能区面积
		CString temp;
		temp.Format(_T("面积合计%.3lfm{\\H0.7x;\\S2^;}，"), sarea);//sarea用于存储指定层高和计容系数下的功能区及外半墙面积
		info += temp; info += it->first; info += _T("\\P\t");//
		dzdarea += sarea;//累加当前类别非扣岛功能区面积和外半墙面积
	}
	if(!wbqisAnn)
	{
		CString temp;
		...
		info += temp;
		info += _T("\\P\t");
		dzdarea += wbqjzmj;//dzdarea用于存储分层平面中所有功能区和外半墙的总面积
	}
	

	double cwarea = 0.0;//所有车位面积
	...
	if (fabs(cwarea) > EPS)//如果车位总面积为零，则不输出
	{
		CString temp;
		temp.Format(_T("车位面积合计 = %.3lfm{\\H0.7x;\\S2^;}，"), cwarea);//追加车位面积合计
		info += temp;
	}
	CString temp;
	temp.Format(_T("总面积合计 = %.3lfm{\\H0.7x;\\S2^;}"), dzdarea + cwarea);//追加总面积合计
	info += temp;
	return true;
}
...
{
	AcDbObjectIdArray cwids;//车位ID集合
	AcGePoint2dArray nodes;//id的所有节点
	GetPlList(aname(id), nodes);
	TCHAR cwlayer[255] = { 0 };
	_stprintf(cwlayer, _T("%s"), m_cw._layer);
	if (!ssFromNodes(cwids, nodes, 1, 1.0, _T("*POLYLINE"), cwlayer)) return false;//框选所有的多段线
	int cwCount = 0;//id包围的车位总数目
	map<CString, int> carmp;
	for (int indx = 0; indx < cwids.length(); ++indx)
	{
		AcDbObjectId cwid = cwids[indx];//获得当前的车位ID号
		TCHAR cwsm[255] = { 0 };
		ReadXdata(aname(cwid), _T("CWSM"), 1, cwsm);//读取XData中的车位数目
		int icwsm = _tstoi(cwsm);
		cwCount += icwsm;//累加车位数目
		TCHAR cwlx[255] = { 0 };
		ReadXdata(aname(cwid), _T("CWLX"), STRING_TYPE, cwlx);//读取XData中的车位类型
		if (carmp.find(cwlx) != carmp.end())//因为存在子车位，所以车位个数可能为零，不能简单加1
			carmp[cwlx] += icwsm;
		else//因为存在子车位，所以车位个数可能为零，不能简单赋值1
			carmp.insert(make_pair(cwlx, icwsm));
	}
	CString zjtcbw;
	zjtcbw.Format(_T("本层总计停车泊位%d个，"), cwCount);
	info += zjtcbw;
	if (carmp.size() != 0)
	{
		info += _T("其中");
		...
		{
			CString temp;
			temp.Format(_T("%s%d个，"), it->first, it->second);
			info += temp;
		}
	}
...
#pragma region 统计信息
	...
	{
		cxxs = pdb.getDXXTableInfo();//DXX表中的“建筑占地面积”列中的全部值相加即为竣工验收报告书中的建筑占地面积
		for (int idx = 0; idx < (int)cxxs.size(); idx++)
		{
			MStr cxx = cxxs.at(idx);
			jzzdmj += _tstof(cxx[_T("建筑占地面积")]);
...
{
	AcGePoint2dArray nodes;//id所有节点
	GetPlList(aname(id), nodes);//获得id所有节点
	AcDbObjectIdArray dgnqids;//所有岛ID集合，不同的ID集合尽可能使用不同的名称，或者在使用之前清空集合
	TCHAR layer[255] = { 0 };//用于储存图层名称
	_stprintf(layer, _T("%s"), m_gnq._layer);//复制功能区图层
	ssFromNodes(dgnqids, nodes, 1, 0.5, _T("*POLYLINE"), layer);//WP选择功能区实体
	dgnqids.remove(id);
	for (int idx = 0; idx < dgnqids.length(); idx++)//遍历岛功能区
	{
		AcDbObjectId did = dgnqids.at(idx);
		TCHAR disKouDao[255] = { 0 };//扣岛属性
		ReadXdata(aname(did), _T("扣岛"), STRING_TYPE, disKouDao);
		if (_tcscmp(disKouDao, _T("false")) == 0)continue;
		double area = 0.0;//几何面积
		GetEntArea(aname(did), area);//获得几何面积
		TCHAR jrxs[255] = { 0 };//计容系数
		ReadXdata(aname(did), _T("计容系数"), STRING_TYPE, jrxs);
		jrArea -= area * _tstof(jrxs);//扣除岛功能区计容面积
		TCHAR jzxs[255] = { 0 };//面积系数
		ReadXdata(aname(did), _T("面积系数"), STRING_TYPE, jzxs);
		jzArea -= area * _tstof(jzxs);//扣除岛功能区建筑面积
	}
	AcDbObjectIdArray dcwids;
	_stprintf(layer, _T("%s"), m_cw._layer);//复制车位图层
	ssFromNodes(dcwids, nodes, 1, 0.5, _T("*POLYLINE"), layer);//WP选择车位实体
	for(int idx = 0; idx < dcwids.length(); ++idx)//遍历岛车位
	{
		AcDbObjectId did = dcwids.at(idx);
		TCHAR pr[255] = { 0 };//岛车位产权属性
		ReadXdata(aname(did), _T("PR"), STRING_TYPE, pr);//读取岛车位产权属性
		if (_tcscmp(pr, _T("1")) == 0)continue;//只扣除多产权岛车位面积
		double area = 0.0;//岛车位几何面积
		GetEntArea(aname(did), area);//计算岛车位几何面积
		jzArea -= area;//扣除岛车位几何面积
		jrArea -= area;//扣除岛车位几何面积
	}
}

bool XMJGHouse::getFunctionArea(AcDbObjectId &id, FunctionHAH &hah)//计算功能区、车位面积
{
	TCHAR layer[20] = { 0 };//存储功能区或车位图层
	GetEntLayer(id, layer);//获得id的图层
	if(m_gnq._layer.CompareNoCase(layer) == 0)//如果是功能区
	{
		TCHAR value[255] = { 0 };
		double area = 0.0;//功能区几何面积
		GetEntArea(aname(id), area);//计算功能区边界线所围面积
		ReadXdata(aname(id), _T("计容系数"), 0, value);//读取功能区的计容系数
		hah.m_jrxs = _tstof(value);//赋值计容系数
		hah.m_jrmj = area * _tstof(value);//计算计容面积
		ReadXdata(aname(id), _T("面积系数"), 0, value);//读取功能区的面积系数
		hah.m_jzmj = area * _tstof(value);//计算建筑面积
		removeSubFunction(id, hah.m_jzmj, hah.m_jrmj);
		ReadXdata(aname(id), _T("功能区高度"), 0, value);
		hah.m_cg = (float)_tstof(value);
		ReadXdata(aname(id), _T("功能区简码"), 0, value);
		hah.m_bh.Format(_T("%s"), value);
		ReadXdata(aname(id), _T("功能区编号"), 0, value);
		hah.m_bh += value;
	}
	else if(m_cw._layer.CompareNoCase(layer) == 0)//如果是车位
	{
...
void XMJGHouse::getWaiBanQiangArea(const AcDbObjectId &id, double &jzmj, double &jrmj)
{
	...
	AcGePoint2dArray node;//id标识的多段线所有节点
	GetPlList(aname(id), node);//获得id标识的多段线的所有节点
	AcDbObjectIdArray wids;//建筑外墙线ID集合
	AcDbObjectIdArray nids;//建筑内墙线ID集合
	double warea = 0.0;//建筑外墙线包围面积之和
	double narea = 0.0;//建筑内墙线包围面积之和
	TCHAR layer[255] = { 0 };//图层
	_stprintf(layer, _T("%s"), wqx._layer);//将CString类型的外半墙图层转化为TCHAR字符串
	if(!ssFromNodes(wids, node, 1, 1.0, _T("*POLYLINE"), layer)) return;//选择id标识的多段线内的所有外墙线
	_stprintf(layer, _T("%s"), nqx._layer);//将CString类型的内半墙图层转化为TCHAR字符串
	if(!ssFromNodes(nids, node, 1, 1.0, _T("*POLYLINE"), layer)) return;//选择id标识的多段线内的所有内墙线
	for(int idx = 0; idx < wids.length(); ++idx)//计算所有外墙线所围面积之和
	{
		double area = 0;
		GetEntArea(aname(wids[idx]), area);
		warea += area;
	}
	for(int idx = 0; idx < nids.length(); ++idx)//计算所有内墙线所围面积之和
	{
		double area = 0.0;
		GetEntArea(aname(nids[idx]), area);
		narea += area;
	}
	jzmj += (warea - narea); jrmj += (warea - narea);//将id标识的多段线内的外半墙面积追加到jzmj和jrmj中
}
